...
from .squad_metric import compute_score
...
def compute_sqa_metrics(eval_preds: EvalOnGeneratePrediction):
    preds, refs = eval_preds.predictions.tolist(), eval_preds.references.tolist()
    import json
    refs = [json.loads(r) for r in refs]
    results = compute_score(dataset=refs, predictions=preds)
    return {
