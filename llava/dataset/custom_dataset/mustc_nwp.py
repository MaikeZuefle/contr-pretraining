...
#
        for language in tqdm(config.languages, desc="Loading MuSTC NWP dataset"):
            ...
            for split, info in partitions.items():
                ...
                dataset = load_dataset(
                    "parquet",
                    data_files={
                        f"{split}": f"{config_datapath}/{source}-{target}_{split}_processed.parquet",
                    },
                ...
                if self.no_punctuation:
                    ...
                    print(
                        f"Removing punctuation"
                    )

                if not self.audio_nwp:
                    raise ValueError("This is not and Audio NWP Training but yet MustC-audio-nwp has been used as training data!")
    ...
    def remove_punctuation(self, example):
        ...
        word_boundaries =  [[sec1, sec2, re.sub(regex, "", word)] for [sec1, sec2, word] in word_boundaries_orig if re.sub(regex, "", word)]
