...
from llava.constants import AUDIO_TOKEN_INDEX, IMAGE_TOKEN_INDEX
...
def tokenizer_mm_token(
    ...
    # TODO: maybe extend to both audio and image
    if "<audio>" in prompt:
        prompt_chunks = [
            tokenizer(chunk).input_ids for chunk in prompt.split("<audio>")
        ]
        token_index = audio_token_index
    elif "<image>" in prompt:
        prompt_chunks = [
            tokenizer(chunk).input_ids for chunk in prompt.split("<image>")
        ]
        token_index = image_token_index
    else:
    ...
    if token_index is not None:
        for x in insert_separator(prompt_chunks, [token_index] * (offset + 1)):
